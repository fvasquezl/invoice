...
<div class="classic-business bg-white p-12" style="min-height: 297mm;">
        ...
        <div class="flex justify-between items-center">
            <div>
                @if ($invoice->company_logo)
                    <img src="{{ Storage::url($invoice->company_logo) }}" alt="{{ $invoice->company_name }}"
                        class="h-20 mb-3">
                @endif
    ...
    <div class="grid grid-cols-2 gap-8 mb-8">
        ...
        <div class="space-y-6">
            {{-- From --}}
            <div>
                <div class="text-xs font-bold uppercase tracking-wide mb-2" style="color: {{ $primaryColor }};">
                    From
                </div>
            ...
            {{-- Bill To --}}
            <div>
                <div class="text-xs font-bold uppercase tracking-wider mb-2" style="color: {{ $primaryColor }};">
                    Bill To
                </div>
                <div class="text-sm text-gray-800 space-y-1">
        ...
        <table class="w-full">
            <thead>
                <tr class="text-white" style="background-color:{{ $primaryColor }}">
                    <th class="text-center py-3 px-4 font-semibold text-sm uppercase">
                        Description
                    </th>
                    <th class="text-center py-3 px-4 font-semibold text-sm uppercase w-20">
                        Qty
                    </th>
                    <th class="text-center py-3 px-4 font-semibold text-sm uppercase w-28">
                        Rate
                    </th>
                    <th class="text-center py-3 px-4 font-semibold text-sm uppercase w-32">
                        Amount
                    </th>
                </tr>
            </thead>
    ...
    <div class="flex justify-end mb-8">
        <div class="w-96">
            <table class="w-full text-sm">
                <tr class="border-b border-gray-300">
                    <td class="py-2 text-gray-700">Subotal:</td>
    ...
    <div>
        @if ($invoice->notes)
            <div class="border-2 p-4" style="border-color: {{ $primaryColor }};">
                <h4 class="text-sm font-bold uppercase tracking-wide mb-2" style="color: {{ $primaryColor }};">
                    Notes
                </h4>
            </div>
        @endif
